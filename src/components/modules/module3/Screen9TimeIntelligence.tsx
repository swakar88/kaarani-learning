...
export default function Screen9TimeIntelligence({ onNext, onPrev, screenIndex, totalScreens }: ScreenProps) {
  ...
  // 4 blocks: header(0), prerequisite(1), function picker + detail(2), YoY tip(3)
  const { step, next, isComplete, blockClass, tapLabel } = useBlockReveal(4);

  const narrationScript = [
    `Time intelligence is where DAX truly shines above any other tool. Most business questions are about time — and DAX makes them trivial.`,
    `Before you can use any time intelligence function, your model needs a proper Date table. Not just dates scattered across your fact table — a dedicated dimension with one row per day, no gaps, covering your full data range. Ours is already set up.`,
    `Six functions cover every time-based question you'll encounter. Year-to-date, quarter-to-date, same period last year, date comparisons, rolling windows. Tap each function to see how it's used in our ${flavor.label} model.`,
    `Here's a powerful pattern: add both your regular measure and the same-period-last-year measure to a single line chart. Instantly you have a year-over-year comparison visual — one of the most requested charts in any business report. One line of DAX.`,
  ];
  ...
  return (
    <ModuleLayout moduleId={3} screenIndex={screenIndex} totalScreens={totalScreens}
      ...
      onPrev={onPrev}
      primaryAction={{ label: "DAX variables →", onClick: onNext, disabled: !isComplete }}
    >
      <div className="max-w-2xl mx-auto">
        ...
        {/* Block 1 — The Date table requirement */}
        ...
        {/* Block 2 — Function picker + detail */}
