...
export default function Screen4CardsKPIs({ onNext, onPrev, screenIndex, totalScreens }: ScreenProps) {
  ...
  const narrationScript = [
    "Cards and KPI visuals are the first things executives read. They give an at-a-glance number — no axes, no comparison, just the metric.",
    `Here are example KPI cards for your ${flavor.label} dashboard. Every dashboard should open with three to five cards summarising the key metrics.`,
    "Three card types: the basic Card for a single number, the KPI visual for actual versus target with a trend arrow, and the Multi-row Card for multiple measures in one compact block.",
    "Always use the New Card visual — not the old one. It supports multiple measures, conditional formatting, and reference labels. And always place cards at the very top of your dashboard.",
  ];
  ...
  return (
    <ModuleLayout moduleId={4} screenIndex={screenIndex} totalScreens={totalScreens}
      ...
      primaryAction={isComplete
        ? { label: "Tables & matrices →", onClick: onNext }
        : { label: "Tables & matrices →", onClick: onNext, disabled: true }}
    >
      <div className="max-w-2xl mx-auto">
        ...
        {/* Live KPI demo */}
        <div className={`${blockClass(1)} rounded-2xl p-5 mb-5`} style={{ backgroundColor: "#FFFFFF", border: "1.5px solid #E8E8E8" }}>
        ...
        {/* Card types */}
        <div className={`${blockClass(2)} grid grid-cols-3 gap-3 mb-5`}>
          {[
