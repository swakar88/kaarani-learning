...
export default function Screen10CourseComplete({ onNext, onPrev, screenIndex, totalScreens }: ScreenProps) {
  const { selectedFlavor, completedModules, unlockVoice } = useKaarani();
  ...
  const narrationScript = [
    "You did it. You came in knowing your domain data — and you're leaving knowing Power BI, data modelling, DAX, visualisation, and cloud deployment.",
    "Look at the skills you've earned across six modules. That combination makes you genuinely rare. Most people can analyse data or communicate it. You can now do both.",
    "Your next steps: the PL-300 exam is your first formal goal. Build a portfolio project with real data. Join the Power BI community. And keep going — every dataset you touch will teach you something new. Go build something that matters.",
  ];
  ...
  return (
    <ModuleLayout moduleId={5} screenIndex={screenIndex} totalScreens={totalScreens}
      ...
      primaryAction={isComplete
        ? { label: " Restart course", onClick: () => { if (typeof window !== "undefined") window.location.href = "/module/0"; } }
        : { label: " Restart course", onClick: () => { if (typeof window !== "undefined") window.location.href = "/module/0"; }, disabled: true }}
    >
          ...
          <div className="flex flex-col gap-2">
            {NEXT_STEPS.map((s, i) => (
              <div key={i} className="flex items-start gap-3">
                <p className="text-xs" style={{ color: "#111827" }}>{s.text}</p>
              </div>
            ))}
          </div>
        </div>
