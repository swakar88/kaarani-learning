...
export default function Screen1DesktopToWeb({ onNext, onPrev, screenIndex, totalScreens }: ScreenProps) {
  ...
  const narrationScript = [
    `Congratulations on making it to the final module! Everything you've built — your ${flavor.label} data model, your DAX measures, your beautiful visualisations — needs to get to your audience.`,
    "The journey has five steps: you build in Desktop, you publish with one click, the report lives in the Power BI Service cloud, you configure sharing, and your colleagues view it on any device.",
    `For ${flavor.label}, the plan is: publish the ${scenario.reportName} to the ${scenario.workspaceName} workspace, then share it with ${scenario.primaryAudience}. Power BI Desktop is free — Pro at ten dollars per user per month is needed for sharing.`,
  ];
  ...
  return (
    <ModuleLayout moduleId={5} screenIndex={screenIndex} totalScreens={totalScreens}
      ...
      primaryAction={isComplete
        ? { label: "Tour the Service →", onClick: onNext }
        : { label: "Tour the Service →", onClick: onNext, disabled: true }}
    >
      <div className="max-w-2xl mx-auto">
        ...
        {/* Journey steps */}
        <div className={`${blockClass(1)} flex flex-col gap-3 mb-6`}>
          {JOURNEY.map(j => (
