...
export default function Screen3Publish({ onNext, onPrev, screenIndex, totalScreens }: ScreenProps) {
  ...
  return (
    <ModuleLayout moduleId={5} screenIndex={screenIndex} totalScreens={totalScreens}
      ...
      onPrev={onPrev}
      primaryAction={{ label: "Workspaces & apps →", onClick: onNext }}
    >
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <button key={i} type="button" onClick={() => setPublishStep(Math.max(publishStep, i + 1))}
                ...
                style={{ borderColor: done ? M_COLOR : active ? M_COLOR : "#E5E7EB", backgroundColor: "#FFFFFF", opacity: i > publishStep + 1 ? 0.4 : 1 }}>
                <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                  style={{ backgroundColor: done ? M_COLOR : active ? M_COLOR : "#E5E7EB", color: (done || active) ? "#FFFFFF" : "#9CA3AF" }}>
                  {done ? "✓" : i + 1}
                </div>
                <div>
                  <p className="font-semibold text-sm" style={{ color: "#111827" }}>{s.label}</p>
                  {(done || active) && <p className="text-xs mt-0.5" style={{ color: "#6B7280" }}>{s.note}</p>}
                </div>
                {active && <span className="ml-auto text-xs px-2 py-1 rounded-lg font-bold" style={{ backgroundColor: "#FFFFFF"}}>Tap →</span>}
              </button>
            );
          })}
        </div>
