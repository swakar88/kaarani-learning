...
export default function Screen5RLS({ onNext, onPrev, screenIndex, totalScreens }: ScreenProps) {
  ...
  const narrationScript = [
    "Row-Level Security — RLS — is how you make one report show different data to different people. A sales manager sees only their region. An executive sees everything.",
    "The setup has four steps: define roles in Desktop, add a DAX filter expression, publish to the Service, then assign users to roles. The report is identical — the data changes based on who's logged in.",
    `Here are the RLS roles for ${flavor.label} — tap each role to see the DAX filter expression that controls what data each role sees.`,
    "Three common mistakes to avoid: forgetting to assign users to roles in the Service, testing with your own account (dataset owners see everything), and creating 50 separate roles instead of using dynamic RLS with USERNAME().",
  ];
  ...
  return (
    <ModuleLayout moduleId={5} screenIndex={screenIndex} totalScreens={totalScreens}
      ...
      primaryAction={isComplete
        ? { label: "Scheduled Refresh →", onClick: onNext }
        : { label: "Scheduled Refresh →", onClick: onNext, disabled: true }}
    >
      <div className="max-w-2xl mx-auto">
        ...
        {/* How it works */}
        <div className={`${blockClass(1)} flex gap-2 mb-4`}>
          {["Define roles in Desktop", "Add DAX filter", "Publish", "Assign users in Service"].map((s, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1 p-2 rounded-xl text-center"
              ...
              <p className="text-[10px] leading-tight" style={{ color: "#111827" }}>{s}</p>
            </div>
          ))}
        </div>
        ...
        <div className={`${blockClass(2)} mb-4`}>
          ...
          <div className="rounded-2xl p-4 animate-fade-in-up" style={{ backgroundColor: "#FFFFFF", border: "1px solid #E5E7EB" }}>
