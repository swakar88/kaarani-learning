...
export default function Screen7Sharing({ onNext, onPrev, screenIndex, totalScreens }: ScreenProps) {
  ...
  const narrationScript = [
    "Sharing in Power BI has four main methods — and choosing the right one saves you headaches. Each has a different audience size, security model, and setup complexity.",
    "Tap each sharing method to see the pros, cons, and best use case. The App is almost always the right answer for more than five people.",
    `For ${flavor.label}: the recommended approach is publishing as an App to ${scenario.primaryAudience}. Never use Publish to Web for internal reports — it's completely public with zero security.`,
  ];
  ...
  return (
    <ModuleLayout moduleId={5} screenIndex={screenIndex} totalScreens={totalScreens}
      ...
      primaryAction={isComplete
        ? { label: "Mobile Layout →", onClick: onNext }
        : { label: "Mobile Layout →", onClick: onNext, disabled: true }}
    >
