...
export default function Screen8MobileLayout({ onNext, onPrev, screenIndex, totalScreens }: ScreenProps) {
  ...
  const narrationScript = [
    "Power BI has a dedicated mobile layout editor in Desktop. Your desktop canvas might have 16 visuals — your mobile layout should have just 4 KPI cards.",
    "Executives checking their phone at the airport want the one number that matters, not a scrollable wall of charts. Toggle between Desktop and Mobile view, then tap KPIs to add them to the mobile layout.",
    "Mobile best practices: stack visuals vertically, show three to five KPIs maximum, use large tap targets, and avoid complex matrices, small text, and scatter charts. Also consider data-driven alerts — they push a notification when a KPI crosses a threshold.",
  ];
  ...
  return (
    <ModuleLayout moduleId={5} screenIndex={screenIndex} totalScreens={totalScreens}
      ...
      primaryAction={isComplete
        ? { label: "Microsoft Fabric →", onClick: onNext }
        : { label: "Microsoft Fabric →", onClick: onNext, disabled: true }}
    >
      <div className="max-w-2xl mx-auto">
        ...
        <div className={`${blockClass(2)} grid grid-cols-2 gap-3`}>
